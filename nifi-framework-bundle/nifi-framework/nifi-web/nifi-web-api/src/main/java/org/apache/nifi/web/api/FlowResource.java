...
public class FlowResource extends ApplicationResource {
    ...
    @Consumes(MediaType.WILDCARD)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("extension-registry-client-definition/{group}/{artifact}/{version}/{type}")
    @Operation(
            summary = "Retrieves the Extension Registry Client Definition for the specified component type.",
            description = NON_GUARANTEED_ENDPOINT,
            responses = {
                    @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = ExtensionRegistryClientDefinition.class))),
                    @ApiResponse(responseCode = "400", description = "NiFi was unable to complete the request because it was invalid. The request should not be retried without modification."),
                    @ApiResponse(responseCode = "401", description = "Client could not be authenticated."),
                    @ApiResponse(responseCode = "403", description = "Client is not authorized to make this request."),
                    @ApiResponse(responseCode = "404", description = "The extension registry client definition for the coordinates could not be located.")
            },
            security = {
                    @SecurityRequirement(name = "Read - /flow")
            }
    )
    public Response getExtensionRegistryClientDefinition(
            @Parameter(
                    description = "The bundle group",
                    required = true
            )
            ...
            @PathParam("version") String version,
            @Parameter(
                    description = "The extension registry client type",
                    required = true
            )
            @PathParam("type") String type
    ) throws InterruptedException {
        ...
        final ExtensionRegistryClientDefinition entity = serviceFacade.getExtensionRegistryClientDefinition(group, artifact, version, type);
