...
@Tags({"monitoring", "workflow", "site", "site to site"})
@CapabilityDescription("TODO")
@Restricted(
        restrictions = {
                @Restriction(
                        requiredPermission = RequiredPermission.EXPORT_NIFI_DETAILS,
                        explanation = "Provides operator the ability to send sensitive details contained in framework events to any external system.")
        }
)
@DefaultSchedule(strategy = SchedulingStrategy.TIMER_DRIVEN, period = "1 min")
public class SiteToSiteWorkflowMonitoringReportingTask extends AbstractSiteToSiteReportingTask {
    ...
    static final PropertyDescriptor WORKFLOW = new PropertyDescriptor.Builder()
        .name("Workflow")
        .description("Value to use fot the name of the workflow to be monitored by this reporting task.")
        .required(true)
        .expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY)
        .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
        .build();

    static final PropertyDescriptor PROCESSOR = new PropertyDescriptor.Builder()
        .name("Processor UUID")
        .description("UUID of a processor belonging to the workflow to monitor.")
        .required(true)
        .expressionLanguageSupported(ExpressionLanguageScope.VARIABLE_REGISTRY)
        .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
        .build();
    ...
    private void initializeReportingTask(final ReportingContext context) {
        if(hasBeenInitialized) {
            return;
        }

        // processor uuid provided by the user in the reporting task configuration
        final String processorUUID = context.getProperty(PROCESSOR).evaluateAttributeExpressions().getValue();
        ...
        // once we have our processor, create the workflow
        processorStatus.get
    ...
    public SiteToSiteWorkflowMonitoringReportingTask() throws IOException {
