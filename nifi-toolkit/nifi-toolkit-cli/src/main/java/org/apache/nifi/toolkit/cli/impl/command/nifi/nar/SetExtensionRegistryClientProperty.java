...
public class SetExtensionRegistryClientProperty extends AbstractNiFiCommand<StringResult> {

    public SetExtensionRegistryClientProperty() {
        super("set-extension-reg-client-property", StringResult.class);
    }

    @Override
    public String getDescription() {
        return "Sets a property on an extension registry client.";
    }
    ...
    public StringResult doExecute(final NiFiClient client, final Properties properties)
        ...
        final ExtensionRegistryClientEntity existingRegClient = client.getControllerClient().getExtensionRegistryClient(regClientId);
        if (existingRegClient == null) {
            throw new CommandException("Extension registry client does not exist for id " + regClientId);
        }

        final ExtensionRegistryClientDTO existingRegClientDTO = existingRegClient.getComponent();
        final ExtensionRegistryClientDTO newRegClientDTO = new ExtensionRegistryClientDTO();
        ...
        final ExtensionRegistryClientEntity newRegClient = new ExtensionRegistryClientEntity();
        ...
        final ExtensionRegistryClientEntity updatedRegistryClient = client.getControllerClient().updateExtensionRegistryClient(newRegClient);
